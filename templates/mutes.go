...
<article class="note muted-user" id="muted-{{.Pubkey}}">
  <div class="note-author">
    ...
    <form method="POST" action="/mute" class="inline-form mute-action" h-post h-target="#muted-{{.Pubkey}}" h-swap="outer" h-confirm="{{i18n "confirm.unmute"}}">
