...
type RelaysConfig struct {
	WriteOnlyRelays    []string `json:"writeOnlyRelays"`
	...
	HandlerRelays      []string `json:"handlerRelays"`
...
func loadRelaysConfigFromFile() *RelaysConfig {
	...
	slog.Info("loaded relays configuration",
		...
		"handler", len(config.HandlerRelays),
		"blossom", len(config.BlossomServers),
		"write_only", len(config.WriteOnlyRelays))
	return &config
}
...
// GetDefaultRelays returns the default relay list for general queries
func GetDefaultRelays() []string {
...
// GetNostrConnectRelays returns the relay list for NIP-46 NostrConnect
func GetNostrConnectRelays() []string {
	config := GetRelaysConfig()
...
// GetSearchRelays returns the relay list for search queries
...
// GetPublishRelays returns the relay list for publishing events
func GetPublishRelays() []string {
	config := GetRelaysConfig()
...
// GetProfileRelays returns the relay list for profile lookups
func GetProfileRelays() []string {
	config := GetRelaysConfig()
...
// GetHandlerRelays returns the relay list for NIP-89 handler discovery
func GetHandlerRelays() []string {
	config := GetRelaysConfig()
	if len(config.HandlerRelays) > 0 {
		return config.HandlerRelays
	}
	return getDefaultRelaysConfig().HandlerRelays
}

// GetBlossomServers returns the list of blossom servers for BUD-10 URI resolution
func GetBlossomServers() []string {
	config := GetRelaysConfig()
	if len(config.BlossomServers) > 0 {
		return config.BlossomServers
	}
	return getDefaultRelaysConfig().BlossomServers
}

// GetWriteOnlyRelays returns the list of relays that only accept writes (EVENT) but not reads (REQ)
func GetWriteOnlyRelays() []string {
	config := GetRelaysConfig()
	return config.WriteOnlyRelays
}
