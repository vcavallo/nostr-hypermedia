...
// EncodeNEvent encodes a nevent from event ID (hex), optional author pubkey (hex), and optional relay hints
func EncodeNEvent(eventIDHex string, authorHex string, relayHints []string) (string, error) {
	...
	// Event ID (type 0/special): 32 bytes - must be first
	tlvData = append(tlvData, tlvTypeSpecial, 32)
	tlvData = append(tlvData, eventIDBytes...)

	// Relay hints (type 1): variable length
	for _, relay := range relayHints {
		if relay == "" {
			continue
		}
		relayBytes := []byte(relay)
		if len(relayBytes) > 255 {
			continue // Skip overly long relay URLs
		}
		tlvData = append(tlvData, tlvTypeRelay, byte(len(relayBytes)))
		tlvData = append(tlvData, relayBytes...)
	}

	// Author pubkey (type 2): 32 bytes (optional)
	if authorHex != "" {
		authorBytes, err := hex.DecodeString(authorHex)
		if err == nil && len(authorBytes) == 32 {
			tlvData = append(tlvData, tlvTypeAuthor, 32)
			tlvData = append(tlvData, authorBytes...)
		}
	}
	...
	return Bech32Encode("nevent", data5bit)
}
...
// EncodeNProfile encodes an nprofile from pubkey (hex) and optional relay hints
func EncodeNProfile(pubkeyHex string, relayHints []string) (string, error) {
	pubkeyBytes, err := hex.DecodeString(pubkeyHex)
	if err != nil {
		return "", err
	}
	...
	// Pubkey (type 0/special): 32 bytes - must be first
	tlvData = append(tlvData, tlvTypeSpecial, 32)
	tlvData = append(tlvData, pubkeyBytes...)
	...
	return Bech32Encode("nprofile", data5bit)
}
