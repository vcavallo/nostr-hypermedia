...
func Nip44EncryptWithNonce(plaintext string, conversationKey []byte, nonce []byte) (string, error) {
	...
	cipherStream, err := chacha20.NewUnauthenticatedCipher(chachaKey, chachaNonce)
	if err != nil {
		return "", err
	}
	ciphertext := make([]byte, len(padded))
	cipherStream.XORKeyStream(ciphertext, padded)
...
func Nip44Decrypt(payload string, conversationKey []byte) (string, error) {
	...
	cipherStream.XORKeyStream(padded, ciphertext)
